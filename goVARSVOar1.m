%% Estimates quasi-real-time runs of VAR-SVO-AR(1) model
...
labelSampling = strcat(labelSampling, '-SVOar1', sprintf('max%d', SVOmaxscale));
...
parfor ndxT = 1 : Njumpoffs % parfor
    ...
    [PAI_all, PHI_all, invA_all, SVrho_all, sqrtht_all, ...
        ...
        = mcmcVARSVOar1(thisT, MCMCdraws, p, np, thisdata, ydates, ...
    ...
    SVdraws                       = NaN(N,Tdata,MCMCdraws);
    stochvol                      = NaN(N, Tdata, MCMCdraws);
    stochvol(:,p+1:thisT, :)      = sqrtht_all ./ SVOscale_all;
    for m=1:MCMCdraws
        invA     = invA_all(:,:,m);
        for t=1:Tdata
            SVdraws(:,t,m) = sqrt(diag(invA*diag(stochvol(:,t,m).^2)*invA'));
