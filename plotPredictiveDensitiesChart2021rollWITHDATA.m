...
for n = ylist
    ...
    for ndxT = ndxRange
        ...
        h2tail = plot(theseFcstdates,squeeze(fcstTailsSV(n,:,:,ndxT)), 'r--', 'linewidth', 2);
        
        hanni(3)   = plot(theseFcstdates,fcstMidSVOt(n,:,ndxT), 'b-.', 'linewidth', 3);
        h3tail = plot(theseFcstdates,squeeze(fcstTailsSVOt(n,:,:,ndxT)), 'b-.', 'linewidth', 2);
        ...
        delete(hanni(3))
        delete(h3tail)
        wrapthisfigure(thisfig, sprintf('%s-%s-%s-WITHTITLEDATA-roll2', thisPlotLabel, ncode{n}, datestr(thisdateT, 28)), wrap, [], [], [], [], true)
        delete(hanni(2))
        delete(h2tail)
        ...
        delete(hanni(3))
        delete(h3tail)
        wrapthisfigure(thisfig, sprintf('%s-%s-%s-WITHDATA-roll2', thisPlotLabel, ncode{n}, datestr(thisdateT, 28)), wrap, [], [], [], [], true)
        delete(hanni(2))
        delete(h2tail)
        wrapthisfigure(thisfig, sprintf('%s-%s-%s-WITHDATA-roll1', thisPlotLabel, ncode{n}, datestr(thisdateT, 28)), wrap, [], [], [], [], true)
...
for n = ylist
    ...
    for ndxT = ndxRange
        ...
        h2tails = plot(theseFcstdates,squeeze(fcstTailsSVOt(n,:,:,ndxT)), 'b-.', 'linewidth', 2);
        
        hanni(3)   = plot(theseFcstdates,fcstMidSVdummy(n,:,ndxT), 'm-', 'linewidth', 3); % 'color', [0 .5 0],
        h3tails = plot(theseFcstdates,squeeze(fcstTailsSVdummy(n,:,:,ndxT)), 'm-', 'linewidth', 1);
