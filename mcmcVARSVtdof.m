function [PAI_all, PHI_all, invA_all, sqrtht_all, ...
    SVtscalelog2_all, ...
    fcstYdraws, fcstYhat, fcstYhatRB, fcstSVdraws] = mcmcVARSVtdof(thisT, MCMCdraws, p, np, data0, ydates0, ...
...
while m < MCMCreps % using while, not for loop to allow going back in MCMC chain
    ...
    if m == 0
        ...
        %         PREVdraw.SVtscalelog2  = zeros(N, T);
    ...
    [Vol_states, ~, eta,  sqrtht, SVtscalelog2] = ...
        StochVoltdof(yresid2', logyresid2', Vol_states, sqrtPHI_, Vol_0mean, Vol_0vcvsqrt, ...
        SVtdof, gridKSC, gridKSCt, N, T, rndStream);
    ...
    if m > burnin;
        ...
        % prepare igamma draws for t
        % tdof          = repmat(SVtdof, 1, fcstNhorizons, Ndraws);
        % SVtscaledraws = sqrt(tdof ./ chi2rnd(SVtdof)); 
        % todo: chi2rnd, or even trnd seems faster than igammadraw, 
        % ... but need to use rndStream
        
        %         sqrtigd       = sqrt(igammadraw(SVtdof, SVtdof, fcstNhorizons * Ndraws, rndStream));
        %         SVtscaledraws = reshape(sqrtigd, N, fcstNhorizons, Ndraws);
        
        % draw t-random numbers 
