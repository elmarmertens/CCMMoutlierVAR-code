function [PAI_all, PHI_all, invA_all, sqrtht_all, ...
    ...
    fcstYdraws, fcstYhat, fcstYhatRB, fcstSVdraws, fcstLogscoreDraws, fcstLogscoreDrawsRB] = mcmcVARSVObar(thisT, MCMCdraws, p, np, data0, ydates0, ...
    minnesotaPriorMean, doTightPrior, doRobustPrior, ...
    SVobaralpha, SVobarbeta, SVobarmaxscale, ...
...
% set up outliers
if isempty(SVobarmaxscale)
    SVobarmaxscale = 10;
end
SVobarstates.Ngrid      = SVobarmaxscale - 1;
SVobarstates.uniformkernel = ones(1,SVobarmaxscale - 1) / (SVobarmaxscale - 1);
SVobarstates.values     = 1 : SVobarmaxscale;
SVobarstates.squaredvalues  = (1 : SVobarmaxscale).^2;
SVobarstates.log2values = 2 * log(SVobarstates.values);
...
SVobarprob_all = NaN(1, MCMCdraws);   
SVobarscale_all = NaN(1, T, MCMCdraws);
...
while m < MCMCreps % using while, not for loop to allow going back in MCMC chain
    ...
    if m == 0
        ...
        PREVdraw.SVobarprob    = .1;
        PREVdraw.SVobarscale   = ones(N, T);
    ...
    SVobarprob    = PREVdraw.SVobarprob;
    SVobarscale   = PREVdraw.SVobarscale;
    ...
    %% NEW STEP: draw obar
    yresid      = RESID*A_';
    zresid      = yresid ./ (sqrtht ./ SVobarscale'); % recall that sqrtht includes SVobarscale
     
    [SVobarscale, SVobarprob] = obarGibbsdraw(zresid, SVobarprob, SVobaralpha, SVobarbeta, SVobarstates, rndStream);

    %% STEP 2d and STEP 1: Draw mixture states and then volatility states
    ytilderesid = yresid' ./ SVobarscale;
    ...
    sqrtht = (exp(Vol_states/2) .* SVobarscale)'; %compute sqrtht^0.5 from volatility states, needed in step 2b
    ...
    if m > burnin;
        ...
        SVobarprob_all(:,thisdraw)    = SVobarprob;
        SVobarscale_all(:,:,thisdraw) = SVobarscale; 
        ...
        % prepare outlier CDF
        obarSVpdf   = [(1 - SVobarprob),  repmat(SVobarprob / SVobarstates.Ngrid, 1, SVobarstates.Ngrid)];
        obarSVcdf   = cumsum(obarSVpdf, 2);   % checkdiff(obarSVcdf(:,end), ones(N,1));
        obarSVcdf   = permute(obarSVcdf, [1 3 2]);
        
        
        % draw random numbers
        zdraws  = randn(rndStream, N, fcstNhorizons, Ndraws);
        udraws  = rand(rndStream, 1, fcstNhorizons, Ndraws);
        ...
        for nn = 1 : Ndraws
            ...
            % generate outlier
            ndx         = sum(udraws(:,:,nn) > obarSVcdf, 3) + 1;
            obarSV      = SVobarstates.values(ndx);
            
            SVdraws     = exp(logSV * 0.5);
            fcstSVdraws(:,:,nn,thisdraw)    = obarSV .* SVdraws;
            ...
            condlogscores = -.5 * (logtwopi + logSV(:,1) + SVobarstates.log2values ...
                + (Ydev ./ SVdraws(:,1) ./ SVobarstates.values).^2);
            % check
            ...
            fcstLogscoreDrawsRB(nn,thisdraw) = sum(log(sum(exp(condlogscores - maxlogscore) .* obarSVpdf, 2)) + maxlogscore);
    ...
    PREVdraw.SVobarprob  = SVobarprob;
    PREVdraw.SVobarscale = SVobarscale;
