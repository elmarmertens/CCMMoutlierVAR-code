%% Estimates quasi-real-time runs of VAR-SV-obar model (with common SV outlier)
...
labelSampling = strcat(labelSampling, '-SVobar', sprintf('max%d', SVOmaxscale));
...
%% SVobar prior
SVobaralpha = 1 / ( 4 * np) * 10 * np;
SVobarbeta  = 10 * np - SVobaralpha;
...
if ~doLoMem
    ...
    drawsSVobarprob    = NaN(1, MCMCdraws, Njumpoffs);
    drawsSVobarscale   = NaN(1, Tdata, MCMCdraws, Njumpoffs);
...
parfor ndxT = 1 : Njumpoffs % parfor
    ...
    [PAI_all, PHI_all, invA_all, sqrtht_all, ...
        ...
        ydraws, yhat, yhatRB, fcstSVdraws, logscoredraws, logscoredrawsRB] ...
        = mcmcVARSVObar(thisT, MCMCdraws, p, np, thisdata, ydates, ...
    ...
    SVdraws                       = NaN(N,Tdata,MCMCdraws);
    stochvol                      = NaN(N, Tdata, MCMCdraws);
    stochvol(:,p+1:thisT, :)      = sqrtht_all ./ SVobarscale_all;
    for m=1:MCMCdraws
        invA     = invA_all(:,:,m);
        for t=1:Tdata
            SVdraws(:,t,m) = sqrt(diag(invA*diag(stochvol(:,t,m).^2)*invA'));
    ...
    if ~doLoMem
        ...
        drawsSVobarprob(:,:,ndxT)    = SVobarprob_all;
        
        % prepare dummy to make parfor work
        dummy                      = NaN(1, Tdata, MCMCdraws);
        dummy(:, p+1:thisT,:)      = SVobarscale_all;
        drawsSVobarscale(:,:,:,ndxT)  = dummy;
    end
end
...
if ~doLoMem
    ...
    % SVOprob
    midSVOprob   = squeeze(mean(drawsSVobarprob, 2));
    tailsSVOprob = permute(prctile(drawsSVobarprob, [25 75], 2), [3 2 1]);
    
    n = 1;
    thisfig = figure;
    hold on
    h1 = plot(ydates(Tjumpoffs), midSVOprob, 'k-d', 'linewidth', 2);
    plot(ydates(Tjumpoffs), tailsSVOprob, 'k--', 'linewidth', 1)
    ...
    wrapthisfigure(thisfig, sprintf('SVobarprob'), wrap)
end
