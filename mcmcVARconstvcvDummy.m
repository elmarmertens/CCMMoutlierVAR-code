function [PAI_all, SIGMA_all, sqrtht_all, ...
    fcstYdraws, fcstYhat, fcstYhatRB] = mcmcVARconstvcvDummy(thisT, MCMCdraws, p, np, data0, ydates0, ...
    minnesotaPriorMean, doTightPrior, dummyPrecision, ...
    ndxYIELDS, ELBbound, check_stationarity, ...
    fcstNdraws, fcstNhorizons, rndStream, doprogress) %#ok<INUSL>
...
%% define Coviddummy
Ncovid     = sum(ydates >= datenum(2020,3,1));
dummyCovid = zeros(thisT, Ncovid);
for n = 1 : Ncovid
    dummyCovid(:,n) = ydates == datenum(2020,2 + n,1);
end
...
X = [ones(Nobs-p,1) lags(p+1:Nobs,:) dummyCovid(p+1:end,:)];

% trim Y
Y = data(p+1:end,:);
% update pointers
[T,K]      = size(X);
Klagreg    = K - 1 - Ncovid; % number of lag regressors (without intercept)
...
if dummyPrecision > 0
    sigma_Dummy = NaN(Ncovid,N);
    for i=1:N
        sigma_Dummy(:,i) = AR_s2(i,i) * theta(1) * theta(2) / dummyPrecision; % note: high dummyPrecision increases shrinkage of prior
    end
else
    sigma_Dummy = NaN(Ncovid,N); % repmat(realmax, Ncovid,N); % repmat(1 / eps, Ncovid,N);
end

% Pai~N(vec(MU_pai),OMEGA_pai), equation 7.
OMEGA_pai   = diag(vec([sigma_const; reshape(diag(Pi_pv),Klagreg,N); sigma_Dummy])); % prior variance of Pai
MU_pai      = [zeros(1,N);reshape(Pi_pm,Klagreg,N);zeros(Ncovid,N)];                   % prior mean of Pai
...
iV(isnan(iV)) = 0;
iVb_prior=iV*vec(MU_pai);    % inverses of prior matrices
EYEn = eye(N);
%% start of MCMC loop
...
while m < MCMCreps % using while, not for loop to allow going back in MCMC chain
    ...
    if m == 0
        ...
        trainingT           = T; % to make sure dummies are included sum(ydates < datenum(2020,3,1)) - p;
    ...
    if m > burnin;
        ...
        for nn = 1 : Ndraws
            ...
            fcstA(ndxfcstY, :)       = PAI(1:end-Ncovid,:)'; 
            fcstX0 = Xjumpoff(1:end-Ncovid); 
