...
fcstTailsSVoutmiss2  = NaN(size(fcstTailsCONST));
fcstMidSVoutmiss2    = NaN(size(fcstMidCONST));
ndx                            = find(ismember(Tjumpoffs, thismat.Tjumpoffs));
fcstTailsSVoutmiss2(:,:,:,ndx)  = squeeze(thismat.fcstYquantiles2(:,:,ndxCI,:));
fcstMidSVoutmiss2(:,:,ndx)      = thismat.fcstYmedian2;
...
for n = ylist
    ...
    for ndxT = ndxRange
        ...
        h3tail = plot(theseFcstdates,squeeze(fcstTailsSVdummy(n,:,:,ndxT)), 'm-', 'linewidth', 1);
        ...
        hd = plot(theseFcstdates, theseRealized, 'd', 'color', darkgreen, 'linewidth', 3);
        plot(theseFcstdates, theseRealized, ':', 'color', darkgreen, 'linewidth', 3);
        ...
        delete(hanni(3))
        delete(h3tail)
        wrapthisfigure(thisfig, sprintf('%s-%s-%s-WITHTITLEDATA-roll1', thisPlotLabel, ncode{n}, datestr(thisdateT, 28)), wrap, [], [], [], [], true)
        
    end
end

%% plot SVO vs SVoutmiss vs SVoutmiss2
if doSVoutmiss2
    ndxRange = find(ydates(Tjumpoffs) >= datenum(2020,3,1));
    ndxRange = ndxRange(:)';
    ...
    close all
    thisPlotLabel = 'predictiveDensityChartOutmiss2';
    for n = ylist
        ...
        for ndxT = ndxRange
            ...
            hanni(3)   = plot(theseFcstdates,fcstMidSVoutmiss2(n,:,ndxT), 'm-', 'linewidth', 3); % 'color', [0 .5 0],
            plot(theseFcstdates,squeeze(fcstTailsSVoutmiss2(n,:,:,ndxT)), 'm-', 'linewidth', 1);
            ...
            ht = title(sprintf('%s -- %s', Ylabels{n}, datestr(thisdateT, datestrfmt)));
            %         wrapthisfigure(thisfig, sprintf('%s-%s-%s-WITHTITLE', thisPlotLabel, ncode{n}, datestr(thisdateT, 28)), wrap, [], [], [], [], true)
            
            hl = legend(hanni, prettylabelSVoutmiss, prettylabelSVOt, 'SV-OutMiss ALT', ...
                'location', 'best');
            ...
            %
            %         hl = legend([hanni; hd], prettylabelSVoutmiss, prettylabelSVOt, 'SV-outmiss ALT',  'realized',  ...
            %             'location', 'best');
