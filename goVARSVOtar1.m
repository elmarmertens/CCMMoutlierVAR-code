%% Estimates quasi-real-time runs of VAR-SVO-t-AR(1) model
...
labelSampling = strcat(labelSampling, '-SVOtar1', sprintf('max%d', SVOmaxscale));
...
%% SVO prior
SVOpriorobs = 10 * np;
SVOalpha    = 1 / (10 * np) * SVOpriorobs; % 1 every ten years (!)
...
fcstYmvlogscoreRB = NaN(1,Njumpoffs); % one-step ahead only
...
if ~doLoMem
    ...
    drawsSVOprob    = NaN(N, MCMCdraws, Njumpoffs);
    drawsSVOscale   = NaN(N, Tdata, MCMCdraws, Njumpoffs);
...
parfor ndxT = 1 : Njumpoffs % parfor
    ...
    [PAI_all, PHI_all, invA_all, SVrho_all, sqrtht_all, ...
        ...
        = mcmcVARSVOtar1(thisT, MCMCdraws, p, np, thisdata, ydates, ...
    ...
    SVdraws                       = NaN(N,Tdata,MCMCdraws);
    stochvol                      = NaN(N, Tdata, MCMCdraws);
    stochvol(:,p+1:thisT, :)      = sqrtht_all ./ SVOscale_all .* exp(-.5 * SVtscalelog2_all);
    for m=1:MCMCdraws
        invA     = invA_all(:,:,m);
        for t=1:Tdata
            SVdraws(:,t,m) = sqrt(diag(invA*diag(stochvol(:,t,m).^2)*invA'));
    ...
    maxlogscoredraw               = max(logscoredrawsRB);
    fcstYmvlogscoreRB(:,ndxT)     = log(mean(exp(logscoredrawsRB - maxlogscoredraw))) + maxlogscoredraw;
    ...
    if ~doLoMem
        ...
        drawsSVOprob(:,:,ndxT)    = SVOprob_all;
        
        % prepare dummy to make parfor work
        dummy                      = NaN(N, Tdata, MCMCdraws);
        dummy(:, p+1:thisT,:)      = SVOscale_all;
        drawsSVOscale(:,:,:,ndxT)  = dummy;
...
if ~doLoMem
    ...
    midPrior = mean(priordraws);
    tailsPrior = prctile(priordraws, [25 75]);
    
    % SVOprob
    midSVOprob   = squeeze(mean(drawsSVOprob, 2));
    tailsSVOprob = permute(prctile(drawsSVOprob, [25 75], 2), [3 2 1]);
    
    for n = 1 : N
        thisfig = figure;
        hold on
        h1 = plot(ydates(Tjumpoffs), midSVOprob(n,:), 'k-d', 'linewidth', 2);
        plot(ydates(Tjumpoffs), tailsSVOprob(:,:,n), 'k--', 'linewidth', 1)
        
        h2 = plothorzline(midPrior, [], 'b-', 'linewidth', 2);
        plothorzline(tailsPrior(1), [], 'b--')
        plothorzline(tailsPrior(2), [], 'b--')
        
        legend([h1 h2], 'posterior', 'prior')
        xtickdates(ydates(Tjumpoffs))
        
        title(sprintf('SV-O prob. for %s (in QRT)', Ylabels{n}))
        
        wrapthisfigure(thisfig, sprintf('SVOprob%d', n), wrap)
    end
end
%% SVOscale
close all
if ~doLoMem
    % collect QRT SVOscales
    thisSVOscaleQRT       = NaN(N,length(ydates));
    thisSVOscaleQRTtails  = NaN(N,2,length(ydates));
    theseSVOscaleVintages = squeeze(median(drawsSVOscale, 3)); % returns N x Tdata x Njumpoff
    theseSVOscaleVintagesTails = prctile(drawsSVOscale, [normcdf(1) 90], 3); % returns N x Tdata x Njumpoff
    for v = 1 : Njumpoffs
        ndx = find(~isnan(theseSVOscaleVintages(1,:,v)),1, 'last'); % note: sufficient to check only for first element
        if isnan(theseSVOscaleVintages(1,ndx,v))
            error houston
        end
        thisSVOscaleQRT(:,ndx)          = theseSVOscaleVintages(:,ndx,v);
        thisSVOscaleQRTtails(:,:,ndx)   = squeeze(theseSVOscaleVintagesTails(:,ndx,:,v));
    end
    
    thisSVOscaleT   = theseSVOscaleVintages(:,:,end);
    
    % plot SVO scale QRT vs final
    ...
    for n = 1 : N
        ...
        thisfig = figure;
        hold on
        h1 = bar(ydates(Tjumpoffs), thisSVOscaleT(n,Tjumpoffs));
        h2 = plot(ydates(Tjumpoffs), thisSVOscaleQRT(n,Tjumpoffs), 'rd', 'linewidth', 5);
        h3 = plot(ydates(Tjumpoffs), squeeze(thisSVOscaleQRTtails(n,1,Tjumpoffs)), 'r--', 'linewidth', 2);
        h4 = plot(ydates(Tjumpoffs), squeeze(thisSVOscaleQRTtails(n,2,Tjumpoffs)), 'r:', 'linewidth', 2);
        ylim([0 SVOmaxscale])
        xtickdates(bardates)
        legend([h1 h2 h3 h4], 'Final (median)', 'QRT (median)', 'QRT (upper 84)', 'QRT (upper 90)')
        title(sprintf('SV-O scale for %s', Ylabels{n}))
        
        wrapthisfigure(thisfig, sprintf('SVOscale%d', n),wrap)
...
for n = 1 : N
    ...
    set(ax, 'fontsize', 16)
    hold on
    plot(thesedates, drawsSVmid(n,:,end), '-', 'linewidth', 2)
    plot(thesedates, drawsLambdaSVmid(n,:,end), '--', 'linewidth', 2)
    ylim([0 max(ylim)])
    xtickdates(thesedates)
    ...
    wrapthisfigure(thisfig, sprintf('finalSV%s', ncode{n}), wrap)
...
h3 = plot(ydates(Tjumpoffs), fcstYmvlogscoreRB , 'b-.', 'linewidth', 2);
legend([h1 h2 h3], 'mixture approx.', 'Gaussian approx.', 'RB mixture', 'location', 'best')
xtickdates(ydates(Tjumpoffs))
subplot(2,1,2)
hold on
h1 = plot(ydates(Tjumpoffs), fcstYmvlogscore, 'r-', 'linewidth', 2);
% h2 = plot(ydates(Tjumpoffs), fcstYmvlogscore2 , 'k--', 'linewidth', 2);
h3 = plot(ydates(Tjumpoffs), fcstYmvlogscoreRB , 'b-.', 'linewidth', 2);
% legend([h1 h3], 'mixture approx.', 'Gaussian approx.', 'RB mixture', 'location', 'best')
xtickdates(ydates(Tjumpoffs))
