function [PAI_all, PHI_all, invA_all, SVrho_all, sqrtht_all, ...
    ...
    fcstYdraws, fcstYhat, fcstYhatRB, fcstSVdraws, fcstSVoutliers, fcstLogscoreDraws, fcstLogscoreDrawsRB] = mcmcVARSVOar1(thisT, MCMCdraws, p, np, data0, ydates0, ...
...
while m < MCMCreps % using while, not for loop to allow going back in MCMC chain
    ...
    [Vol_states, Vol_mean, Vol_dev, ~, sqrtht, SVOlog2, SVOprob, SVOscale] = ...
    StochVolOutlierAR1(logy2', Vol_states, hrho,sqrtPHI_, Vol_0mean, Vol_0vcvsqrt, ...
     SVOlog2, SVOprob, SVOalpha, SVObeta, SVOstates, ...
    gridKSC, gridKSCt, N, T, rndStream);
