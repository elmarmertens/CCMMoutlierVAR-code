...
switch dummyType
    ...
    otherwise
        error('dummyType <<%s>> not reognized', dummyType)
end
...
parfor ndxT = 1 : Njumpoffs % parfor
    ...
    if Compute_diagnostics
        ...
        DiagnosticsCONST(PAI_all,SIGMA_all,N,thisK,MCMCdraws);
    end
    ...
    if ~doLoMem
        if strcmpi(dummyType, 'CONSTCOVIDEACH')
...
if ~doLoMem && ~strcmpi(dummyType, 'CONSTsansCOVID')
    ...
    for nn = 1 : Ndummy
        for n = 1 : N
            ...
            thisCI  = squeeze(prctile(drawsDummy(nn,n,:,:), [25 75], 3));
