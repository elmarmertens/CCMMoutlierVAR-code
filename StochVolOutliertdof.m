function [h, h0, hshock, SV, outlierlog2Draws, outlierProb, outlierScaleDraws, SVtScalelog2Draws] = ...
...
chi2draws          = chi2rnd(repmat(SVtdof, 1, T));
SVtScalelog2Draws  = log(scalePosterior) - log(chi2draws);
% igdraws          = scalePosterior ./ chi2rnd(repmat(SVtdofDraws, 1, T));
% SVtScaleDraws    = sqrt(igdraws);
