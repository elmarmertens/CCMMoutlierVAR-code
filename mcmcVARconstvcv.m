function [PAI_all, SIGMA_all, sqrtht_all, ...
    fcstYdraws, fcstYhat, fcstYhatRB, fcstLogscoreDraws] = mcmcVARconstvcv(thisT, MCMCdraws, p, ~, data0, ydates0, ...
...
while m < MCMCreps % using while, not for loop to allow going back in MCMC chain
    ...
    if m > burnin;
        ...
        for nn = 1 : Ndraws
            ...
            %             muX          = fcstA * fcstX0;
            %             muY          = muX(ndxfcstY);
            %             sqrtOmegaY   = cholSigma;
            %             logdetOmegaY = 2 * sum(log(diag(sqrtOmegaY)));
            %             % checkdiff(logdetOmegaY, log(det(cholSigma * cholSigma')));
            %
        ...
        % logscore
        % note: logscore is the same for all Ndraws, for consistency with
        % other mcmc's copying it Ndraws times into output structure
        
        % update VAR companion form and iterate
        fcstA(ndxfcstY, :)       = PAI';
        fcstX0 = Xjumpoff;
        ...
        Ydev = sqrtOmegaY \ (yrealized(:,1) - muY);
        fcstLogscoreDraws(:,thisdraw) = -.5 * (Nlogtwopi + logdetOmegaY + sum(Ydev.^2));
